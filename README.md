...
| 단계 | 지표 |
|---|---|
| **S4 성분 분리** | [S4-01] 상관 — 분절 내 Pearson → 분절 간 `ρ̄ = mean_s\|ρ\|`, `σ = std_s\|ρ\|`(ddof=1), 행별 최댓값 표시<br>[S4-02] 정규화 RMSE — 분절 내 표준화 → 분절 내 RMSE → 분절 간 평균±SD(ddof=1), 행별 최솟값 표시<br>MAD·r² 병기, 부호 분포 별도 기록. p값·명명·해석 없음 |
