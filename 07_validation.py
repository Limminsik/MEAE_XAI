...
03에서 확정한 체크포인트를 **재학습 없이** 외부 ECG 에 적용해, 우리 파이프라인이
MIT-BIH 밖에서도 그대로 도는지 본다. 학습·마스킹·복원 경로를 하나도 바꾸지 않는다.

    python 07_validation.py --run <최종런> --source vitaldb

지도학습이라 **인코더가 무엇을 담는지 배정으로 정해져 있다** — 그래서 외부에서도
A 성분차감(x - ŝ_bw - ŝ_ma - ŝ_em)을 그대로 만들 수 있다. 자기지도 갈래에서는 어느
인코더가 잡음인지 사후에 정해야 했던 부분이다.
...
(분절 내 표준화 → |r| · RMSE_norm · MAD → 분절 간 평균 ± SD, ddof=1).
  ...
  A 성분차감   x - ŝ_bw - ŝ_ma - ŝ_em — 04·05·06 과 같은 정의

**성능은 주장할 수 없다.** 참값이 없으므로 "얼마나 깨끗해졌나"를 잴 방법이 없다.
낼 수 있는 것은 ① 성분이 어떤 주파수 성격을 갖는가 ② A 가 입력과 얼마나 다른가
③ 파형이 눈으로 어떻게 보이는가 뿐이다.
...
────────────────────────────────────────────────────────────────────────
  ...
  figures/components_top{1..10}.png   입력·재구성·성분 K개 (재구성 유사도 상위)
  figures/overlay_top{1..10}.png      입력 위에 A 성분차감 겹침 (05 와 같은 형식)
  figures/spectrum.png                성분별 PSD
  restore.csv                         복원 5종이 입력과 얼마나 다른가 (|r|·RMSE·SD비)
  sqi_summary.csv                     **SQI 5종** — 참값 없이 재는 품질 지수
  morphology.csv                      임상 형태 지표의 **값** (참값이 없어 오차가 아니다)
"""
import argparse
import os
