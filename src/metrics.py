...
#   pSQI   = P(5-20Hz) / P(0-40Hz)       QRS 대역 비중.    잡음이 줄면 **높아진다**
#   kSQI   = 첨도 (4차 모멘트 / 분산²)     잡음 없는 QRS 일수록 높다 (통상 5 이상)
#   bSQI   = |R1 ∩ R2| / |R1 ∪ R2|       서로 다른 두 검출기의 박동 일치 비율
# ================================================================
# 두 지표의 **분모를 0-40 Hz 로 통일**한다. 그래야 "전체 대역 대비 비율"이라는 말이
# 두 지표에서 같은 것을 가리킨다. pSQI 분모는 0-62.5 Hz 였는데, 62.5 Hz 는 125 Hz
# 표본율의 나이퀴스트로 우리 360 Hz 에서는 근거가 없는 경계다.
# 분자는 건드리지 않는다 — basSQI 0-1 Hz, pSQI 5-20 Hz.
SQI_BANDS = {"basSQI": ((0.0, 1.0), (0.0, 40.0)),      # (분자 대역, 분모 대역)
             "pSQI": ((5.0, 20.0), (0.0, 40.0))}
