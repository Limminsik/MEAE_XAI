...
from src.viz import spotcheck, spotcheck_overlay

SEGDIR = "../data/processed/segments"     # baseline 데이터를 읽기 전용으로 쓴다
...
def spotcheck_figs(cfg, names=None, record=None, start=0, n=3, seed=42):
    ...
    if jobs:
        for name in jobs:
            ...
            spotcheck(path, out)
            spotcheck_overlay(path, out.replace(".png", "_overlay.png"))
            print(f"  {sp}  {name}  →  {out} (+_overlay)")
        return
    random.seed(seed)
    for sp in ("train", "val", "test"):
        ...
        out = os.path.join(OUTDIR, f"{sp}_{name}.png")
        spotcheck(f, out)
        spotcheck_overlay(f, out.replace(".png", "_overlay.png"))
