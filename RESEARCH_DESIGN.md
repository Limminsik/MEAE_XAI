...
> **기본 규칙: clean 참조와 최대 상관인 인코더 1개만 남기고 나머지 인코딩 전부를 0으로 치환.**

`configs/noise_encoders.yaml`에 이 규칙이 지목한 **유지 인코더 번호**가 기록되며,
S5·S6는 이를 그대로 읽는다 (§0 원칙 6).

**규칙의 자의성은 아블레이션이 해소한다.** "왜 하필 그 규칙인가"에 대해, 아래 주 실험이
K개 인코더를 **하나씩 전부 개별 검증**하고 나아가 **전수 2^K 마스킹 지도**를 제시하므로
특정 규칙을 변호할 필요가 없다. 독자가 전체 지형을 보고 규칙의 타당성을 직접 확인한다.
...
세 조건은 **단독 마스킹 K개**로 검증하고, 추가로 **전수 2^K 마스킹 지도**를 산출한다
(K=8이면 256, K=4면 16 조합). 규칙 선택의 자의성을 지형 전체로 대체하는 장치다.
