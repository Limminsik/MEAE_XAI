...
| **② RMSE_norm** | 정규화·부호 정렬 후 RMSE. **`= √(2(1−ρ))` 로 ρ의 함수다**(각주 명시). 해석 편의를 위해 병기하며 독립 근거가 아니다 | ①과 같은 정보의 다른 표현 |
| **③ MAD** | 정규화 후 `max\|ẑ_k − r̂\|`. 정규화 후에도 ρ와 독립인 유일한 지표 | 국소 최대 편차 (보완적) |

SSD·PRD도 ②와 마찬가지로 ρ의 함수이므로 싣지 않는다.

**원값 RMSE는 폐기한다.** 성분의 절대 크기가 임의 스케일이라 순위가 뒤집힌다 —
에폭 48 실측에서 enc3이 bw \|r\| 0.602로 1위인데 원값 RMSE는 0.4936으로 8개 중 최하위였다.

**표 1 = 인코더 × 참조 대응표.** 각 칸에 ①②③을 병기하고, 분절 단위 값의
중앙값[IQR]과 평균±SD를 함께 싣는다.
...
| 산출물 | 파일 |
|---|---|
| ① 인코더 × 참조 대응표 (\|r\|·r²·RMSE_norm·MAD) | `stage1_matrix.csv` + `stage1_matrix_note.txt`, `fig1_correspondence.png` |
