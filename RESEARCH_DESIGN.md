...
### 평가 지표 — |r| 과 원값 RMSE 2종

**표 1 = 인코더 × 참조 대응표.** 각 칸에 **상관계수 |r|** 과 **원값 RMSE**를 병기한다.
분절 단위 값의 중앙값[IQR]과 평균±SD를 함께 싣는다.

배율 보정 RMSE는 쓰지 않는다 — `RMSE(β·x̂, ref) = RMS(ref)·√(1−r²)`로 |r|과 대수적으로 중복이다.

### 산출물

| 산출물 | 파일 |
|---|---|
| ① 인코더 × 참조 대응표 (\|r\| + RMSE) | `stage1_matrix.csv`, `fig1_correspondence.png` |
