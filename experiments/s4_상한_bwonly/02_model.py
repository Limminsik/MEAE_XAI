...
def evaluate(model, val, device, batch: int, n_v: int, with_rho: bool, sup_keys=None,
             ...
             sigmas=None, active=None):
    """L_recon^val · **L_sup^val** 과 (필요 시) ρ_k · S. V는 val의 앞 n_v 분절로 고정한다.

    L_sup^val 이 선택 기준이다. 학습 손실과 **같은 정의**로 계산한다 — `active` 로
    일부만 지도하는 런에서는 그 인코더만 세고 같은 수로 나눈다. 선택 기준이 목적함수와
    어긋나면 엉뚱한 에폭이 뽑힌다.
    """
    model.eval()
    pad, K = model.pad_each, model.n_encoders
    act = list(range(K)) if active is None else sorted(active)
    ...
    if sup_keys:
        ...
        # 손실과 같은 정의 — **지도하는 k 에 대해서만** 평균낸다
        out["val_sup"] = float(per[act].mean())
        out["val_sup_per"] = per
    if with_rho:
...
def train(cfg, n_encoders: int, seed: int, tag: str = "", plot_every: int = 1,
          ...
          sup_only=None, group: str = "", out_root: str = "experiments",
    ...
    if any(v is not None for v in (hidden, gamma, gamma2, beta, sup_norm,
        ...
        if sup_only is not None:
            cfg["loss"]["supervise_only"] = [int(v) for v in str(sup_only).split(",")]
    ...
    active = crit.active        # 평가·선정도 같은 집합을 써야 목적함수와 어긋나지 않는다
    ...
    if len(active) < n_encoders:
        free = [k + 1 for k in range(n_encoders) if k not in active]
        act1 = [k + 1 for k in active]
        say(f"  [상한 측정] 지도 항은 enc{act1} 에만 건다 — enc{free} 는 지도 없이 "
            "재구성에만 기여한다")
        say(f"    L_sup 을 {len(active)}개로 나누므로 그 항의 실효 가중치가 "
            f"전체 지도(1/{n_encoders})보다 {n_encoders / len(active):.0f}배 크다 "
            "— 경쟁 제거와 가중치 증가가 함께 작용한다")
    ...
    info = {"run": run, "K": n_encoders, "seed": seed,
            ...
            "supervise_only": cfg["loss"].get("supervise_only"),
            "active_encoders_1based": [k + 1 for k in active],
...
if __name__ == "__main__":
    ...
    p.add_argument("--supervise-only", dest="sup_only", default=None,
                   help="지도 항을 걸 인코더 번호(1부터, 쉼표). 예: 2 = enc2(bw)만. "
                        "비우면 전 인코더 — 본 트리와 같은 계산이다")
    ...
    if a.diagnose:
        ...
        train(load_cfg(a.config), a.k, a.seed, tag=a.tag, max_epoch=a.max_epoch,
              ...
              skip_levels=a.skip_levels, skip_weight=a.skip_weight,
              sup_only=a.sup_only, group=a.group,
